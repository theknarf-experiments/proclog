...
use proclog_ast::*;
use proclog_core::{ConstantEnv, FactDatabase};
use proclog_eval::stratified_evaluation_with_constraints;
use proclog_grounding as grounding;
...
pub fn asp_sat_evaluation(program: &Program) -> Vec<AnswerSet> {
    ...
    let asp_answer_sets: Vec<proclog_asp::AnswerSet> = answer_sets
        .into_iter()
        .map(|as_set| proclog_asp::AnswerSet {
            atoms: as_set.atoms,
        })
        .collect();

    // Apply optimization
    let optimized = proclog_asp::apply_optimization(program, asp_answer_sets, &const_env);
...
fn evaluate_with_sat_encoding(program: &Program) -> Vec<AnswerSet> {
    ...
    let derived_db = if rules.is_empty() {
        ...
        match proclog_eval::stratified_evaluation_with_constraints(&rules, &[], base_facts.clone())
        {
            Ok(db) => db,
            Err(_) => return vec![], // Rules themselves inconsistent
        }
    ...
    for statement in &program.statements {
        if let Statement::ChoiceRule(choice_rule) = statement {
            ...
            let grounded_groups =
                proclog_grounding::ground_choice_rule_split(choice_rule, &derived_db, &const_env);
            ...
            for grounded_atoms in grounded_groups {
                ...
                let min = choice_rule
                    .lower_bound
                    .as_ref()
                    .and_then(|t| proclog_asp::resolve_bound(t, &const_env))
    ...
    for statement in &program.statements {
        if let Statement::Rule(rule) = statement {
            ...
            let body_substs =
                proclog_grounding::satisfy_body(&rule_with_named_vars.body, &extended_db);
