...
use proclog_ast::{Program, Query};
use proclog_core::FactDatabase;
...
fn parse_query(input: &str) -> Result<Query, Vec<ParseError>> {
    proclog_parser::parse_query(input, SrcId::empty())
}
...
fn test_query_after_evaluation() {
    ...
    for statement in &program.statements {
        match statement {
            proclog_ast::Statement::Fact(fact) => {
                facts.insert(fact.atom.clone()).unwrap();
            }
            proclog_ast::Statement::Rule(rule) => {
...
fn test_query_variable_extraction() {
    ...
    let mut facts = FactDatabase::new();
    for statement in &program.statements {
        if let proclog_ast::Statement::Fact(fact) = statement {
            facts.insert(fact.atom.clone()).unwrap();
        }
    }
...
fn test_query_with_integer_constant() {
    ...
    let const_env = proclog_core::ConstantEnv::from_program(&program);
    let mut facts = FactDatabase::new();
...
fn test_query_with_mixed_constants() {
    ...
    let query2 =
        parse_query("?- character(Name, Health, Active, Level).").expect("Failed to parse query");
    let results2 = evaluate_query(&query2, &facts);
        ...
        match statement {
            ...
            proclog_ast::Statement::Rule(rule) => {
                ...
                let substituted_body: Vec<_> = rule
                    ...
                    .iter()
                    .map(|lit| match lit {
                        proclog_ast::Literal::Positive(atom) => {
                            proclog_ast::Literal::Positive(const_env.substitute_atom(atom))
                        }
                        proclog_ast::Literal::Negative(atom) => {
                            proclog_ast::Literal::Negative(const_env.substitute_atom(atom))
                        }
                        proclog_ast::Literal::Aggregate(agg) => {
                            proclog_ast::Literal::Aggregate(agg.clone())
                        }
                    })
                    .collect();

                rules.push(proclog_ast::Rule {
